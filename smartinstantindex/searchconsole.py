...
def fetch_indexed_pages(site_url: str, credentials_json: str, months_back: int = 16, proxy=None) -> set:
...
def fetch_indexed_pages_from_dict(site_url: str, credentials_dict: dict, months_back: int = 16, proxy=None) -> set:
    ...
    while True:
        ...
        if response.status == 403:
            try:
                ...
                detail = content.decode()[:300]
            raise Exception(f"Access denied (403): {detail}\nTried site URL: {site_url}")
        if response.status == 429:
            raise Exception("GSC rate limit reached. Please try again later.")
        if response.status != 200:
            try:
